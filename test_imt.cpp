...
TEST_CASE( "GnuplottableContour struct" )
{
    ...
    CHECK (os.str () == "5.000000000000000e-01 1.250000000000000e+00 7.500000000000000e-01 5.000000000000000e-01 \n");
}

static
vec_t ellipse_polar_form (double e, double phi)
{
...
static
complex_t expi (double phi)
{
    return std::polar(1., phi);
}

static
void trace_ellipse (MinkowskiAccumulator *acc, double e, double num_segments)
{
...
TEST_CASE( "MinkowskiAccumulator struct" )
{
    ...
    SECTION( "shape is a 3:8 ellipse" )
    {
        ...
        CHECK (acc.imt (2) / acc.perimeter () == approx (-0.63070026151848068207).tolerance (1e-6));
        CHECK (acc.imt (3) / acc.perimeter () == approx (0.).tolerance (1e-6));;
        CHECK (acc.imt (4) / acc.perimeter () == approx (0.34844502478902471207).tolerance (1e-6));
        CHECK (acc.imt (6) / acc.perimeter () == approx (-0.18211710641944390473).tolerance (1e-6));
        CHECK (acc.imt (8) / acc.perimeter () == approx (0.092302729744212168710).tolerance (1e-6));
    }

    SECTION ("large square should approach q4=1")
    {
        const int resolution = 200;
        const double ref_area_pix = fsq (resolution) - 4 * (resolution-1.) - .5;
    ...
    SECTION("imt_polygon")
    {
        ...
        {
            vec_t const v0 = {1., 0.}, v1 = cos_sin(TWO_PI/3), v2 = cos_sin(-TWO_PI/3);
            ...
            for (int s : { 2, 3, 4, 5, 6 })
            {
                complex_t const ref_psi = side * (expi(TWO_PI/6 * s)
                    + expi(-TWO_PI/6 * s) + expi(TWO_PI/2 * s));
