...
inline
double atan2 (const vec_t &v)
{
    return std::atan2 (v[1], v[0]);
}

inline
vec_t cos_sin (double phi)
{
...
inline
std::vector <double> logspace (double low, double high, int n = 10,
...
inline
std::vector <double> linspace (double low, double high, int n = 10,
    bool include_endpoint = false)
{
...
inline IntegerRange range(int min, int max_excluded) { return { min, max_excluded }; }
inline IntegerRange range(/* min*/ int max_excluded) { return { 0,   max_excluded }; }
...
struct Datafile
{
    ...
    friend
    Datafile &operator<< (Datafile &df, double x)
    {
        df.os << x << ' ';
        return df;
    }
    ...
    friend
    Datafile &operator<< (Datafile &df, iomanip_t x)
    {
