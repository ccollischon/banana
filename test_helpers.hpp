...
{
    static
    bool ordered (double x, double y, double z)
    {
        return x <= y && y <= z;
    }
    ...
    friend
    void add_contour_segment (CheckPositivelyOriented *, vec_t, vec_t, vec_t)
    {
        // do nothing
    }
};
...
void check_msq_positively_oriented (const PHOTO &ph)
{
...
struct CApprox
{
    ...
    friend
    bool operator== (const complex_t &is, const CApprox &approx)
    {
    ...
    friend
    CApprox operator* (double factor, const CApprox &approx)
    {
        return CApprox (approx.ref_value_.real () * factor,
            approx.ref_value_.imag () * factor, approx.scale_ * factor,
            approx.tolerance_);
    }

    friend
    CApprox operator* (const CApprox &approx, double factor)
    {
        return factor * approx;
    }

    friend
    std::ostream &operator<< (std::ostream &os, const CApprox &approx)
    {
...
inline
CApprox approx (double real, double imag, double scale = 1.)
{
...
inline
CApprox approx (const complex_t &c, double extra_im = 0., double scale = 1.)
{
