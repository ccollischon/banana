...
struct Sersic
{
    ...
    Sersic (double a_ = 1., double b_ = 1., double angle_ = 0., int exponent_ = 1)
        : a (a_), b (b_), exponent (exponent_)
        , cos (std::cos (angle_)), sin (std::sin (angle_))
    {
...
private:
    static double bn (int exponent)
    {
        double bn[] = {
            -1,
            1, 3.67206075, 5.67016119, 7.66924944, 9.66871461
        };
        return bn[exponent];
    }
};
